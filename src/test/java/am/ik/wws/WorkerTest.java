...
class WorkerTest {
	...
	void serve_GET() {
		...
		worker.doServe(req -> {
			...
			assertThat(req.header("host").get()).isEqualTo("127.0.0.1:8080");
			assertThat(req.header("user-agent").get()).isEqualTo("curl/7.88.1");
			assertThat(req.header("accept").get()).isEqualTo("*/*");
	...
	void serve_POST() {
		...
		worker.doServe(req -> {
			...
			assertThat(req.header("user-agent").get()).isEqualTo("curl/7.88.1");
			assertThat(req.header("accept").get()).isEqualTo("*/*");
			assertThat(req.header("content-type").get()).isEqualTo("text/plain");
